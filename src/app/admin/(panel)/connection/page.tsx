  ...
  return (
    <div>
      ...
      >
        ...
        <button onClick={() => { runServerCheck(); runClientCheck(); runImgbbCheck(); }} className="adm-btn adm-btn-line text-xs">
          ↻ Re-run checks
        </button>
      </PageHead>
      ...
      {/* Server REST check */}
      <Card title="Server reachability check (public Firestore REST)" className="mt-4">
        {status.loading ? (
          <Loading />
        ) : status.error ? (
